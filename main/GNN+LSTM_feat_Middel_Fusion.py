...
data = np.load("/Users/a1234/Fraud/gnn/artifacts/node2vec_embeddings.npz")
E      = data["emb"]           # (N_addr, G)
addrID = data["nodes"]         # (N_addr,)
print(df_clean.columns)



emb_df = pd.DataFrame(E, columns=[f"gnn_{i}" for i in range(E.shape[1])])
emb_df["from_addr"] = addrID   # столбец должен совпадать с df_clean

emb_df["from_addr"] = emb_df["from_addr"].astype(str)
...
gnn_cols = [c for c in df_clean.columns if c.startswith("gnn_")]
df_clean[gnn_cols] = df_clean[gnn_cols].fillna(0.0)

print(f"Добавили {len(gnn_cols)} GNN-признаков; df_clean.shape = {df_clean.shape}")
...
# 1. Собираем маску “ненадёжных” значений (NaN или Inf)
mask_train = ~np.isfinite(X_train_lstm)  # True там, где NaN или Inf
mask_val   = ~np.isfinite(X_val_lstm)

# 2. Вычисляем средние по каждой фиче, игнорируя NaN/Inf
#    получим array shape=(n_feat,)
means = np.nanmean(np.where(np.isfinite(X_train_lstm), X_train_lstm, np.nan),
                   axis=(0,1))

# 3. Заменяем “плохие” средние на 0 (или на что-то осмысленное в вашей задаче)
means = np.where(np.isnan(means), 0.0, means)

# 4. Меняем все NaN/Inf на соответствующее среднее
#    np.where(mask_train) вернёт три массива индексов (seq, timestep, feat)
inds_train = np.where(mask_train)
X_train_lstm[inds_train] = means[inds_train[2]]

inds_val = np.where(mask_val)
X_val_lstm[inds_val] = means[inds_val[2]]  # если хотите с теми же средними

# 5. Проверяем
print("Train finite after:", np.isfinite(X_train_lstm).all())
print("Val   finite after:", np.isfinite(X_val_lstm).all())

import numpy as np

print("X_train: min =", X_train_lstm.min(), 
      "max =", X_train_lstm.max(),
      "mean =", X_train_lstm.mean(),
      "std =", X_train_lstm.std())

print("X_val:   min =", X_val_lstm.min(), 
      "max =", X_val_lstm.max(),
      "mean =", X_val_lstm.mean(),
      "std =", X_val_lstm.std())

from sklearn.preprocessing import MinMaxScaler

# 1) узнаём количество признаков
n_seq, seq_len, n_feat = X_train_lstm.shape

# 2) «сплющиваем» для скейлера: (n_seq*seq_len, n_feat)
Xtr_flat = X_train_lstm.reshape(-1, n_feat)
Xvl_flat = X_val_lstm  .reshape(-1, n_feat)

# 3) fit только на train, transform — и train, и val
mm = MinMaxScaler(feature_range=(0, 1))
Xtr_mm = mm.fit_transform(Xtr_flat)
Xvl_mm = mm.transform( Xvl_flat)

# 4) возвращаем 3D-форму
X_train_scaled = Xtr_mm.reshape(X_train_lstm.shape)
X_val_scaled   = Xvl_mm.reshape(X_val_lstm  .shape)

# 5) проверяем
print("Train MM: min =", X_train_scaled.min(), "max =", X_train_scaled.max())
print("Val   MM: min =", X_val_scaled.min(),   "max =", X_val_scaled.max())
...
history = model.fit(
    ...
    validation_data=(X_val_scaled, y_val_lstm),  # <<< здесь тоже scaled
    epochs=200,
    batch_size=32,
