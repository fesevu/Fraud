...
DROP TABLE IF EXISTS `celtic-tendril-459507-q8.fraud_gnn.gnn_transactions`;
CREATE TABLE `celtic-tendril-459507-q8.fraud_gnn.gnn_transactions`
...
WITH
  all_tx AS (
    ...
    SELECT *
    FROM `celtic-tendril-459507-q8.fraud_gnn.gnn_transactions`
    WHERE DATE(block_timestamp) BETWEEN date_from AND date_to
  ),
  scam_edges AS (
    SELECT t.*
    FROM all_tx AS t
    JOIN `celtic-tendril-459507-q8.fraud_gnn.gnn_accounts` AS a1 ON t.src = a1.id
    JOIN `celtic-tendril-459507-q8.fraud_gnn.gnn_accounts` AS a2 ON t.dst = a2.id
